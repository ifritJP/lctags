...
(defun lctags-rename-at ()
  ...
  (let ((buffer (lctags-get-process-buffer t))
	location-info location-list kind new-symbol)
    ...
    (cond
     ...
     ((lctags-xml-get-list location-info 'refsInFunc)
      (message "not support to rename function"))
     (t
      (setq location-list (lctags-xml-get-list location-info 'location))
	...
	(setq new-symbol (read-string "rename?: "
				      (lctags-location-item-get-symbol (car location-list))))
	(save-excursion
	  (dolist (location (lctags-xml-get-list location-info 'location))
	    (goto-char (lctags-location-item-get-point location))
	    (delete-region (point) (lctags-location-item-get-end-point location))
	    (insert new-symbol))
	  (backward-char)
