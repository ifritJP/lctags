...
それは事前に設定したコールグラフの深さ以内の全コールを辿るグラフを静的に生成していました。

これだと、次の問題がありました。

- 辿りたい関数コール以外のパスまで展開されてしまう
- 直ぐに巨大なグラフになってしまう
- 設定された深さのコールを解析&グラフ作成するため時間がかかる
...
コールグラフを表示するための手順を説明します。
  
** emacs の設定
  
emacs を HTTPD と利用するため、
...
- 動的に関数コールを展開/格納可能
  ...
  - 格納したノードは緑で表示されます。
- 関数名の色を状態に応じて変更
  - コールグラフ上の同じ関数が存在する場合、その関数名を緑で表示します。
...
このとき getIndirectFuncList( symbol ) の symbol には、
...
tree レイアウトによって、関数コールの構造が直感的に分かるようになったと思います。
